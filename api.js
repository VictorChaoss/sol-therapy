...
async function mockSession(loss, walletInfo) {
  await new Promise(r => setTimeout(r, 2200));
  const diagnosis = CONDITIONS[Math.floor(Math.random() * CONDITIONS.length)];
  const walletLine = walletInfo ? ` (${walletInfo})` : '';
  return {
    paragraphs: [
      `I see you've come to us having parted ways with <strong>${loss}</strong>${walletLine}. Most humans, when confronted with that kind of number, experience what we in the field call a "come to Jesus" moment. You, and I mean this in the most clinical sense possible, seem to have shown up at the therapy office instead. That is, genuinely, a step forward. Horizontal growth is still growth.`,
...
async function liveSession(loss, walletInfo) {
  const res = await fetch('/api/therapy', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ loss, walletInfo }),
  });
...
// ─── Exported function ────────────────────────────────────────────────────────
export async function getTherapySession(loss, walletInfo = '') {
  if (IS_DEV) {
    return mockSession(loss, walletInfo);
  }
  return liveSession(loss, walletInfo);
}
